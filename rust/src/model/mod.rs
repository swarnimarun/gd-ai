...
pub fn load_audio_waveform<B: Backend>(filename: &str) -> hound::Result<(Vec<f32>, usize)> {
    ...
    // if not sample rate of 16000 resample(using rubato, as windows doesn't provide clean resampling) to 16000
    ...
    return Ok((resampled, sample_rate));
}

pub fn load_whisper_model_file<B: Backend>(
